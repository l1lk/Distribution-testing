...
class Mutators():
    ...
    def image_random_mutate(seed, batch_num):
        ...
        for i in range(batch_num):
            ...
            if changed:
                ...
                label_batches.append(seed.ground_truth)
        return np.asarray(ref_batches), np.asarray(batches), cl_batches, l0_ref_batches, linf_ref_batches, label_batches

    def image_random_mutate_mmd(seed, seed_num):

        clusters_seed = np.load(seed.fname, allow_pickle=True)
        # print("clusters_seed shape {} (mutators.py)".format(clusters_seed.shape)) #--debug--
        # print("fname {} (mutators.py)".format(seed.fname)) #--debug--

        if len(clusters_seed) == 1:
            test = clusters_seed[0]
        else:
            test = clusters_seed[seed_num]
    ...
    # 对输入的图像做一次随机的mutation
    def image_random_mutate_ga(seed):
        '''
        seed: 要突变的图像
        '''
        ref_img = copy.deepcopy(seed) # ref_img: 保留原图像，计算突变对图像的改变程度是否符合要求
        img = seed
        
        # batches中保存了突变后的图像，其他变量来自deephunter，可以不管
        ref_batches = []
        batches = []
        cl_batches = []
        l0_ref_batches = []
        ...
        ref_out, img_out, cl_out, changed, l0_ref, linf_ref = Mutators.mutate_one(ref_img, img, 0, 0, 0)
